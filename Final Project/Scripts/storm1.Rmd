...
#discharge vs CDOM
ggplot(data = FDOM_Discharge_Rain,
       mapping = aes(x = conc, y= Discharge, color = by15))+
  geom_point()
...
Linear model
```{R}
qplot(Rain, Discharge, color = conc, data = FDOM_Discharge_Rain)+
  scale_color_gradient(low = "orange", high = "blue")+
  theme_classic()+
  facet_wrap(~cut_number(conc,4))

pairs(FDOM_Discharge_Rain)

```

Fit and evaluate assumptions
```{R}
FDOM_mlr <- lm(conc ~ Rain + Discharge + month + intensity, data = FDOM_Discharge_Rain)
